...
# ld   -----imm----- --rs1 xxx --rd- 00000 11
# st   --imm-- --rs2 --rs1 xxx -imm- 00001 11 
# add  -func7- --rs2 --rs1 fu3 --rd- 00010 11 
# inv  -func7- --xxx --rs1 fu3 --rd- 00100 11
# beq  --imm-- --rs2 --rs1 fu3 -imm- 01011 11 
# bne  --imm-- --rs2 --rs1 fu3 -imm- 01100 11 
# jmp    ----------imm-------  --rd- 01101 11 
# lui    ----------imm-------  --rd- 01110 11 
...
# ld   -----imm----- --rgB xxx --rgA 00000 11
# st   --imm-- --rgA --rgB xxx -imm- 00001 11 
# add  -func7- --rgC --rgB fu3 --rgA 00010 11 
# inv  -func7- --xxx --rgB fu3 --rgA 00100 11
# beq  --imm-- --rgB --rgA fu3 -imm- 01011 11 
# bne  --imm-- --rgB --rgA fu3 -imm- 01100 11 
# jmp    ----------imm-------  --rgA 01101 11 
# lui    ----------imm-------  --rgA 01110 11
    ...
    for line in code:
        ...
        if cmd:
            if   cmd == "ld":
                ...
                code = f"   {imm_11_0:012b}_{regB:05b}_000_{regA:05b}_0000011"
            elif cmd == "st":
                imm = int_to_twos_complement(value, 12)
                imm_11_5 = get_bits(imm, 5,  11)
                imm_4_0  = get_bits(imm, 0,  4)
                code = f"  {imm_11_5:07b}_{regA:05b}_{regB:05b}_000_{imm_4_0:05b}_0000111"
            elif cmd == "beq":
                ...
                code = f"{imm_12:01b}_{imm_10_5:06b}_{regB:05b}_{regA:05b}_000_{imm_4_1:04b}_{imm_11:01b}_0101111"
            elif cmd == "bne":
                #         imm[12]    imm[10:5]      imm[4:1]   imm[11]            per spec
                # get 13 bit twos complement as we drop the final bit
                imm = int_to_twos_complement(value, 13)
                ...
                imm_4_1  = get_bits(imm, 1,  4)
                imm_11   = get_bits(imm, 11, 11)
                code = f"{imm_12:01b}_{imm_10_5:06b}_{regB:05b}_{regA:05b}_000_{imm_4_1:04b}_{imm_11:01b}_0110011"
            elif cmd == "jmp":
                ...
                code = f"  {imm_20:01b}_{imm_10_1:010b}_{imm_11:01b}_{imm_19_12:08b}_00000_0110111"
            elif cmd == "inv":		
                code = f"  000000_000000_{regB:05b}_000_{regA:05b}_0010011"
            elif cmd == "lui":
                imm = int_to_twos_complement(value, 20)
                code = f"     {imm:020b}_{regA:05b}_0111011"
